...
def rs_fgsm(s_0: np.ndarray,
            pi_net,
            q_net,
            l_inf_norm: float = 0.0075):
    def loss_fn(s_0, s_hat):
        return q_net(s_0, pi_net(s_hat))

    s_0 = th.from_numpy(s_0.astype(np.float32))
    s_hat = fgsm(s_0, loss_fn, 0, l_inf_norm)

    return s_hat


def rs_pgd(s_0: np.ndarray,
           pi_net,
           q_net,
           l_inf_norm: float = 0.0075,
           lr: float = 1e-3,
    ...
    s_0 = th.from_numpy(s_0.astype(np.float32))
    s_hat = pgd(s_0, loss_fn, 0, l_inf_norm, lr, iter_num),
